...
# --- Внешние URL (заполните своими значениями) ---
# URL скрипта проверки зеркал (загружается и выполняется)
MIRROR_CHECK_SCRIPT_URL=""
# URL команды запуска установщика (для подсказки пользователю)
INSTALLER_RESTART_CMD="bash Freepbx17_debian12.sh"
...
# внешний сервис проверки зеркал.
# ===================================================================================
...
# Загружает и выполняет скрипт проверки, анализирует его вывод.
mirror_check_once() {
  local output_file
  output_file="$(mktemp)"

  # Если URL скрипта проверки не задан — пропускаем проверку
  if [[ -z "$MIRROR_CHECK_SCRIPT_URL" ]]; then
  ...
  if ! curl -fsS "$MIRROR_CHECK_SCRIPT_URL" 2>/dev/null \
    | sed '/^read -r -p/,$d' \
...
print_mirror_status() {
  if [ "$mirror_status" = "good" ] && [ "$deb_status" = "good" ]; then
    ...
    echo -e "Все зеркала в норме. ${WHT}($MIRROR_GOOD_COUNT/$MIRROR_MAX проверок пройдено)${NC}"
    ...
    echo -e "${BRED}Не удаётся связаться с сервисом проверки зеркал. Возможно, ваш IP-адрес заблокирован.${NC}"
...
run_mirror_checks() {
  ...
  while [ "$MIRROR_ATTEMPTS" -lt "$MIRROR_MAX" ]; do
    ...
    echo -e "\n${BGRN}Проверка официальных зеркал FreePBX… (попытка $MIRROR_ATTEMPTS/$MIRROR_MAX)${NC}\n"
    mirror_check_once
    print_mirror_status
  ...
  while [ "$MIRROR_OK" = false ]; do
    ...
    case "$mirror_choice" in
      ...
      2)
        ...
        echo
        echo "Вы можете перезапустить установку командой:"
        echo
        echo "  $INSTALLER_RESTART_CMD"
        echo
...
EOF
  ...
  if [ "$IS_HEQET" = false ] && [ "$IS_NONINTERACTIVE" = false ] && [ -f /var/run/reboot-required ]; then
    ...
    echo -e "${WHT}После перезагрузки запустите установщик снова:${NC}"
    echo -e "${BGRN}  $INSTALLER_RESTART_CMD${NC}"
    echo
