...
# Версия самого скрипта установки
SCRIPTVER="1.15"

# Версия Asterisk для установки (по умолчанию — 22; можно переопределить через переменную ASTVERSION)
ASTVERSION=${ASTVERSION:-22}

# Требуемая версия PHP для работы FreePBX
PHPVERSION="8.2"

# Папка, где будут храниться логи процесса установки
...
# Удобная переменная-ссылка на файл лога
log=$LOG_FILE
...
# Зеркало репозитория Debian (по умолчанию — официальное зеркало)
DEBIAN_MIRROR="http://ftp.debian.org/debian"

# Зеркало для NPM (может быть задано через параметр --npmmirror)
NPM_MIRROR=""

# Переменная для хранения кодового имени версии ОС Debian (например, bookworm, trixie)
DEBIAN_OS_VERSION=""

# Определяем версию ОС по файлу /etc/os-release: ищем строку VERSION_CODENAME= и берём значение после неё
...
# Проверка совместимости ОС: скрипт поддерживает только Debian 12 (bookworm)
...
# Функция для исправления репозиториев: заменить stable на bookworm, stable-security на bookworm-security
fix_debian12_repo() {
...
#     Начало установки
################################################################################################################
# Приоритет зеркала (используется в логике скрипта)
MIRROR_PRIO=600

# Получаем полную строку информации о ядре системы (например, версию и архитектуру)
kernel=$(uname -a)

# Получаем имя хоста системы
host=$(hostname)

# Получаем полное доменное имя (FQDN). Если команда не сработает — переменная останется пустой (|| true защищает от ошибки)
...
# Устанавливаем текущий шаг установки (для вывода в лог и прогресс‑бар)
...
# Установка необходимых зависимых пакетов для FreePBX 17
...
# -------------------------------------------------------------------
# Список пакетов для разработки — заголовочные файлы и инструменты сборки
# -------------------------------------------------------------------
DEPDEVPKGS=(
...
# -------------------------------------------------------------------
# Настройка Postfix: ограничиваем прослушивание только localhost (127.0.0.1)
...
# Удаляем неиспользуемые зависимости, чтобы уменьшить размер системы и избежать конфликтов
...
# Подготовка директорий и настройка конфигурации Asterisk
...
# Закомментированная строка: изменение владельца директории /etc/ssl на пользователя asterisk
...
# Установка пакетов, необходимых для работы FreePBX
...
# Активация PHP‑модуля freepbx (подключает конфигурацию FreePBX к PHP)
...
# Перезапуск службы fail2ban для применения возможных новых правил
setCurrentStep "Restarting fail2ban"
systemctl restart fail2ban  >> "$log"


if [ "$nofpbx" ] ; then
  # Если указана опция --nofreepbx — пропускаем установку FreePBX 17
  ...
  # Если задан NPM_MIRROR — устанавливаем переменную окружения для npm
  ...
  # Если требуется только открытый исходный код (опция --opensourceonly) — удаляем коммерческие модули
  if [ "$opensourceonly" ]; then
  ...
  # Если включена поддержка DAHDI — устанавливаем модуль dahdiconfig и настраиваем переменные окружения для Wanpipe
  if [ "$dahdi" ]; then
  ...
  # Устанавливаем все локально доступные модули FreePBX (из кэша или загруженных ранее)
  setCurrentStep "Installing all local modules"
  fwconsole ma installlocal >> "$log"

  # Обновляем все модули FreePBX до последних версий
  setCurrentStep "Upgrading FreePBX 17 modules"
  fwconsole ma upgradeall >> "$log"

  # Перезагружаем конфигурацию и перезапускаем сервисы FreePBX
  setCurrentStep "Reloading and restarting FreePBX 17"
  fwconsole reload >> "$log"
  fwconsole restart >> "$log"

  # Если выбран режим «только открытый исходный код» — удаляем вспомогательные пакеты, нужные только для коммерческих модулей
  if [ "$opensourceonly" ]; then
...
# Завершение процесса установки: обновляем список юнитов systemd
...
# Блокируем обновление ключевых пакетов, чтобы избежать поломок системы при будущих обновлениях
...
# Обновление подписей модулей FreePBX — это нужно для проверки целостности и подлинности модулей
...
# Сообщаем, что установка FreePBX 17 успешно завершена
...
# Если не была указана опция --nofreepbx — выполняем дополнительную проверку самого FreePBX (доступность API, основных модулей и т. п.)
if [ ! "$nofpbx" ] ; then
 check_freepbx
fi
...
# Если FreePBX был установлен (не пропущен через --nofreepbx) — выводим приветственное сообщение (Message of the Day) через fwconsole
if [ ! "$nofpbx" ] ; then
  fwconsole motd
fi
